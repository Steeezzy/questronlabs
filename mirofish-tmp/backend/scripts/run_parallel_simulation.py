...
- 完成模拟后不立即Closing environment，进入等待命令模式
- 支持通过IPC接收Interview命令
- 支持单个Agent采访和批量采访
- 支持远程Closing environment命令
...
Log Structure:
    ...
    └── run_state.json       # Run status (for API query)
"""
...
class ParallelIPCHandler:
    ...
    def _get_env_and_graph(self, platform: str):
        ...
        Args:
            platform: Platform name ("twitter" 或 "reddit")
    ...
    async def process_commands(self) -> bool:
        ...
        print(f"\nReceivedIPC命令: {command_type}, id={command_id}")
        ...
        if command_type == CommandType.INTERVIEW:
            ...
            print("ReceivedClosing environment命令")
...
async def run_twitter_simulation(
    ...
    Args:
        ...
        main_logger: 主日志管理器
        max_rounds: Max simulation rounds(Optional, used to truncate long simulations)
    ...
    for round_num in range(total_rounds):
        ...
        if _shutdown_event and _shutdown_event.is_set():
            if main_logger:
                main_logger.info(f"Received退出信号，在第 {round_num + 1} 轮停止模拟")
            break
...
async def main():
    ...
    parser.add_argument(
        ...
        action='store_true',
        default=False,
        help='模拟完成后立即Closing environment，不进入等待命令模式'
    )
...
def setup_signal_handlers(loop=None):
    """
    Set signal handlers for proper SIGINT/SIGTERM exits
    
    持久化模拟场景：模拟完成后不退出，等待 interview 命令
    当Received终止信号时，需要：
    1. 通知 asyncio 循环退出等待
    2. Allow program to clean up resources cleanly
    3. 然后才退出
    """
