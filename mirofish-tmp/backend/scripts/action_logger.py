"""
Action Logger
Records actions for each Agent during simulation for backend monitoring
...
class PlatformActionLogger:
    ...
    def __init__(self, platform: str, base_dir: str):
        ...
        Args:
            ...
            base_dir: Base path for simulation directory
        """
...
class SimulationLogManager:
    ...
    Manage all logs universally, split by platform
    """
...
class ActionLogger:
    """
    Action Logger（Compatibility with old interface）
    SimulationLogManager is recommended
    """
...
def get_logger(log_path: Optional[str] = None) -> ActionLogger:
    """获取全局日志实例（Compatibility with old interface）"""
