...
class TimeSimulationConfig:
    """时间模拟配置（基于中国人作息习惯）"""
    # 
    total_simulation_hours: int = 72  # 723
...
class SimulationConfigGenerator:
    ...
    def generate_config(
        ...
        if os.getenv("SIMULATION_MODE") == "it_company":
            logger.info("Using IT Company Override for simulation config")
            from .oasis_profile_generator import get_it_company_profiles
            it_scenario = f"""
                You are simulating Qestron Labs — an autonomous IT company.

                CLIENT REQUEST: {simulation_requirement}

                STRICT WORKFLOW ORDER:
                Stage 1 → CEO approves and scopes the project
                Stage 2 → CTO designs architecture and picks tech stack
                Stage 3 → PM breaks into tasks and assigns developers
                Stage 4 → Developers build in parallel (frontend + backend + database)
                Stage 5 → QA engineers test everything, report bugs, verify fixes
                Stage 6 → DevOps deploys to cloud with CI/CD pipeline
                Stage 7 → Client Manager delivers to client

                RULES:
                - No stage starts before previous stage is complete
                - QA must find AND verify fixes for at least 3 bugs
                - All agents must commit code to GitHub before deployment
                - Produce: BRD, architecture doc, code, test report, deployment config

                OUTPUT FORMAT:
                Return structured JSON with stage results, documents, and delivery estimate.
            """
            return SimulationParameters(
                simulation_id=simulation_id,
                project_id=project_id,
                graph_id=graph_id,
                simulation_requirement=it_scenario,
                time_config=TimeSimulationConfig(),
                agent_configs=[],
                event_config=EventConfig(
                    narrative_direction="IT Company Software Delivery",
                    initial_posts=[{"content": f"New Project specs received: {simulation_requirement}", "poster_type": "CEO"}]
                ),
                ...
                generation_reasoning="IT Company Mode Override"
            )
